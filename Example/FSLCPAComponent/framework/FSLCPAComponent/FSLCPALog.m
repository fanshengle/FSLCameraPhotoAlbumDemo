...
{
    if ([self sharedLog].outputLevel == FSLCPALogLevelFATAL)return;
    NSLogv([@"FSLCPAComponent -> " stringByAppendingString:format], argList);
}
...
{
    if ([self sharedLog].outputLevel < FSLCPALogLevelERROR)return;
    va_list argList;
    va_start(argList,format);
    [self log:[@"[ERROR]:\n" stringByAppendingString:format] arguments:argList];
    va_end(argList);
...
{
    if ([self sharedLog].outputLevel < FSLCPALogLevelWARN)return;
    va_list argList;
    va_start(argList,format);
    [self log:[@"[WARN]:\n" stringByAppendingString:format] arguments:argList];
    va_end(argList);
...
{
    if ([self sharedLog].outputLevel < FSLCPALogLevelINFO)return;
    va_list argList;
    va_start(argList,format);
    [self log:[@"[INFO]:\n" stringByAppendingString:format] arguments:argList];
    va_end(argList);
...
{
    if ([self sharedLog].outputLevel < FSLCPALogLevelDEBUG)return;
    va_list argList;
    va_start(argList,format);
    [self log:[@"[DEBUG]:\n" stringByAppendingString:format] arguments:argList];
    va_end(argList);
